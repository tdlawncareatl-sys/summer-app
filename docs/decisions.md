...
rows tagged `School · <name>`, leaving breaks free. Review-before-save, one-tap
clear/re-import. No schema change.
**Alternatives considered:** Recurring weekly class blocks with time-of-day availability;
live iCal/scraping integration with registrar calendars; a separate school_schedules
table; an away-vs-local/commuter question in the flow (built, then cut same day — Tad's
call: importing a school schedule just means "blocked while school is in session," and
anyone it doesn't fit can skip stretches in the review step).
...
guard the shape). Away/home is whole-day only, and the import treats everyone as gone
while school is in session — a commuter who imports will look blocked all semester
unless they skip stretches. Boundary tweaks (early drive home) are day-level edits on
the calendar, not editable in the sheet.
